...
import "../styles/ShowUsuarios.css";


const endpoint = "http://localhost:8000/api";
const endpoint1 = "http://localhost:8000/api";
...
const ShowUsuarios = () => {
  ...
  const getAllPersonas = async () => {
    const response = await axios.get(`${endpoint1}/personas`);
  ...
  // ROLS
    ...
    const getAllRols = async () => {
      const response = await axios.get(`${endpoint2}/rols`);
                    ...
                    <tbody>
                      {usuarios.map((usuario) => (
                        <tr key={usuario.id}>
                          ...
                          <td>
                          {usuario.rol_id == 1 ? "admin":"maestro"}
                           </td>
                          
                          <td>
                            <Link
                              to={`/usuario/edit/${usuario.id}`}
